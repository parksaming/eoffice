    ...
    <div class="container container-list">
        ...
        <div class="row">
            ...
            <div class="col-md-12" style="display: flex;justify-content: space-between;padding: 0;margin-bottom: 18px;">
                <ul class="nav nav-pills">
                    <li class="bg_luanchuyen {{ $status == 'chuaxuly'? 'active' : '' }}">
                        <a href="{{ route('danhsach.vanbanden_donvi', ['status' => 'chuaxuly']) }}">Chưa xử lý</a>
                    </li>
                    <li class="bg_luanchuyen {{ $status == 'dangxuly'? 'active' : '' }}">
                        <a href="{{ route('danhsach.vanbanden_donvi', ['status' => 'dangxuly']) }}">Đang xử lý</a>
                    </li>
                    <li class="bg_luanchuyen {{ $status == 'daxuly'? 'active' : '' }}">
                        <a href="{{ route('danhsach.vanbanden_donvi', ['status' => 'daxuly']) }}">Đã xử lý</a>
                    </li>
                    <li class="bg_luanchuyen {{ (!$status || $status == 'all')? 'active' : '' }}">
                        <a href="{{ route('danhsach.vanbanden_donvi', ['status' => 'all']) }}">Tất cả</a>
                    </li>
                </ul>
                <div>
            ...
            <div class="container container-search {{ ($tukhoa === '' && $trangthai === '' && $linhvucId === '' && $loaivanbanId === '' && $ngaybanhanhtu === '' && $ngaybanhanhden === '' && $ngayguitu === '' && $ngayguiden === '')? 'hidden' : '' }}">
                <div class="row">
                    <div class="col-md-12">
                        <form method="GET" action="{{ route('danhsach.vanbanden_donvi') }}">
                ...
                @if(sizeof($vanbans))
                    ...
                    @foreach ($vanbans as $val)
                        <tr>
                            ...
                            <td><a href="{{route('chitiet_vanban_donvi',$val->vanbanUser_id)}}">{{$val->vanban->soden}}</a>{!! !$val->ngayxem? '<br><i style="font-size:11px;color:red;">Chưa đọc</i>' : '' !!}</td>
                            <td>{{$val->vanban->loaivanban? $val->vanban->loaivanban->name : ''}}</td>
                            <td>{{$val->vanban->donviChuTri? $val->vanban->donviChuTri->name : ''}}</td>
                            <td>
                                <a href="{{route('chitiet_vanban_donvi',$val->vanbanUser_id)}}" title="{{$val->vanban->kyhieu}}">{{$val->vanban->kyhieu}}</a>
                            </td>
                            ...
                            <td class="text-center">
                                <span style="padding:5px 10px"><a href="{{route('chuyen_xu_ly_van_ban_donvi',$val->vanbanUser_id)}}" title="Chuyển xử lý văn bản"><i class="fa fa-paper-plane"></i></a></span>
                            </td>
                            <td class="text-center">
                                <span style="padding:5px 10px"><a href="{{route('chitiet_vanban_donvi',$val->vanbanUser_id)}}" title="Xem chi tiết"><i class="fa fa-eye"> </i></a></span>
                            </td>
                        </tr>
                    @endforeach
                @else
