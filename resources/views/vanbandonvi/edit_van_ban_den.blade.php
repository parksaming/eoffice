...
<div class="container-fuild pdt20">
    <div class="row">
        ...
        <div class="col-sm-12">
            <form id="FormEditVBDen" class="form-input" action="{{route('save.vanban_donvi')}}" method="POST" enctype="multipart/form-data">
                <div class="form-row">
                                    ...
                                    <div class="form-item-c" style="display: flex; flex-direction: column-reverse;">
                                        <select class="form-control chosen" name="donvi_id" {{ (sizeof($userChuTriDaXemVanBanIds) || $vanban->not_have_chutri)? 'disabled' : '' }}>
                                            ...
                                            @foreach ($donvichons as $val)
                                                <option value="{{$val->id}}" {{ $vanban->donvi_id == $val->id? 'selected' : '' }}>{{$val->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-item">
                                    ...
                                    <div class="form-item-c" style="display: flex; flex-direction: column-reverse;">
                                        <select class="form-control chosen" name="donvi_phoihop_ids[]" multiple>
                                            @foreach ($donvichons as $val)
                                                <option value="{{$val->id}}" {{ in_array($val->id, $vanban->donViPhoiHopIdsVal)? 'selected' : '' }} {{ in_array($val->id, $donviPhoiHopDaXemVanBanIds)? 'disabled' : '' }}>{{$val->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
