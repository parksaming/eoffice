    ...
    <div class="container container-list">
        ...
        <div class="row">
                ...
                @if ($type === 'nhan')
                    <ul class="nav nav-pills">
                        <li class="bg_luanchuyen {{ $status == 'chuaxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=chuaxuly' }}">Chưa xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ $status == 'dangxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=dangxuly' }}">Đang xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ $status == 'daxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=daxuly' }}">Đã xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ (!$status || $status == 'all')? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=all' }}">Tất cả</a>
                        </li>
                    </ul>
                @endif
            ...
            <div class="container container-search {{ ($tukhoa === '' && $trangthai === '' && $linhvucId === '' && $loaivanbanId === '' && $ngaybanhanhtu === '' && $ngaybanhanhden === '' && $ngayguitu === '' && $ngayguiden === '')? 'hidden' : '' }}">
                <div class="row">
                    <div class="col-md-12">
                        <form method="GET" action="{{ url('vanbannoibo/danh-sach') }}">
                                ...
                                <div class="col-md-12 vanban">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <p style="padding-top:7px"><b>Nơi ban hành</b></p>
                                                </div>
                                                <div class="col-md-9">
                                                    @php $noibanhanh = isset($_GET['noibanhanh'])? $_GET['noibanhanh'] : '' @endphp
                                                    <input type="text" class="form-control" name="noibanhanh" id="noibanhanh" value={{ $noibanhanh }}>
                                                </div>
                                            </div>
                                        </div>
                ...
                @if(sizeof($vanbans))
                    ...
                    @foreach ($vanbans as $val)
                        <tr>
                            ...
                            <td>{{$val->vanban->loaivanban? $val->vanban->loaivanban->name : ''}}{!! !$val->ngayxem? '<br><i style="font-size:11px;color:red;">Chưa đọc</i>' : '' !!}</td>
                            @if ($type === 'nhan')
                                <td>{!!$val->userGuiVanBan? $val->userGuiVanBan->fullname.'<br>'.($val->userGuiVanBan->donvi? '<i>('.$val->userGuiVanBan->donvi->name.')</i>' : '') : ''!!}</td>
                            @endif
                            <td>
                                <a href="{{ url('vanbannoibo/chi-tiet', $val->vanbanUser_id) }}" title="{{$val->vanban->kyhieu}}">{{$val->vanban->kyhieu}}</a>
                            </td>
                            <td>{{$val->vanban->ngayden}}</td>
                            <td>{{$val->vanban->title}}</td>
                            <td>{{$val->vanban->hanxuly}}</td>
                            @if ($type === 'nhan')
                                <td>
                                    @if ($val->status == 1)
                                        <a href="javascript:;" onclick="showModalChuyenTrangThaiVanBan({{$val->id}})" class="doNotAction lh36">Chưa xử lý</a>
                                    @elseif ($val->status == 2)
                                        <a href="javascript:;" onclick="showModalChuyenTrangThaiVanBan({{$val->id}})" class="doingAction lh36">Đang xử lý</a>
                                    @elseif ($val->status == 3)
                                        <a href="javascript:;" onclick="showModalChuyenTrangThaiVanBan({{$val->id}})" class="doneAction lh36">Đã xử lý</a>
                                    @endif
                                </td>
                            @endif
                            <td>
                            ...
                            <td class="text-center">
                                <span style="padding:5px 10px"><a href="javascript:;" onclick="quyTrinhChuyenTiep('{{$val->vanbanUser_id}}')" title="Xem quy trình chuyển xử lý"><img src="{{ asset('img/workflow.png') }}" alt="Xem chuyển xử lý" style="width: 16px; height: 16px;"></a></span>
                            </td>
                            @if ($type === 'nhan')
                                <td class="text-center">
                                    <span style="padding:5px 10px"><a href="{{ url('vanbannoibo/chuyen-xu-ly', $val->vanbanUser_id)}}" title="Chuyển xử lý văn bản"><i class="fa fa-paper-plane"></i></a></span>
                                </td>
                            @endif
                            <td class="text-center">
                                <span style="padding:5px 10px"><a href="{{ url('vanbannoibo/chi-tiet', $val->vanbanUser_id) }}" title="Xem chi tiết"><i class="fa fa-eye"> </i></a></span>
                            </td>
                        </tr>
                    @endforeach
                @else
