...
@section('main')
        ...
        <div id="exTab1">
            ...
            <div class="tab-content clearfix">
                <div class="tab-pane {{ $tab == 0? 'active' : '' }}" id="1a">
                    <div class="row">
                        <div class="pull-left" style="padding-left: 18px;"><div style="font-size: 17px;background: yellow;margin-top: 10px;"><i class="fa fa-tag"></i> Văn bản nội bộ</div></div>
                        <div class="pull-right btn-custom" style="display: flex">
                            ...
                            @if ($vanbanden->user_id == session('user')['id'])
                                <a class="color-w btn btn-primary mg-l10" href="{{ url('vanbannoibo/sua', $vanbanden->id) }}" title="Sửa văn bản"> Sửa văn bản</a>
                                <a class="color-w btn btn-primary mg-l10" href="javascript:;" onclick="showModalGuiLaiMail()" title="Gửi lại mail"> Gửi lại mail</a>
                            @endif
                            <a class="color-w btn btn-primary mg-l10" href="{{ url('vanbannoibo/chuyen-xu-ly', $vanbanden->id) }}" title="Chuyển xử lý văn bản"> Chuyển xử lý văn bản</a>
                        ...
                        <div class="col-sm-12 pdt20">
                            <table class="table table-bordered">
                                <tbody>
                                <tr>
                                    <th class="row-left">Trích yếu</th>
                                    <th>{{$vanbanden->title}}</th>
                                ...
                                <tr>
                                    <td class="row-left">Ngày ban hành</td>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            foreach($files as $key => $file) {
                                                echo sprintf('<div><a href="%s" target="_blank" title="%s">%s</a></div>', route('dowload.file', [$file,'vanban_id' => $id,'numberFile' => $key]), $file, $file);
                                            }
                                        @endphp
                                    </td>
                                </tr>
                ...
                <div class="tab-pane {{ $tab == 1? 'active' : '' }}" id="2a">
                    <div class="pull-left" style="margin-bottom: 18px;"><div style="font-size: 17px;background: yellow;margin-top: 10px;"><i class="fa fa-tag"></i> Văn bản nội bộ</div></div>
                ...
                <div class="tab-pane {{ $tab == 2? 'active' : '' }}" id="3a">
                    <div class="pull-left" style="margin-bottom: 18px;"><div style="font-size: 17px;background: yellow;margin-top: 10px;"><i class="fa fa-tag"></i> Văn bản nội bộ</div></div>
                    @include('vanban.chi_tiet_van_ban_den_tab_y_kiens', ['vbxuly' => $vbxuly])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function guiLaiMail() {
            ...
            $.post("{{ route('gui_lai_mail', [$vbxuly->vanbanUser_id]) }}", {_token:$('meta[name="csrf-token"]').attr('content')}, function (res) {
                loading_hide();
                if (!res.error) {
