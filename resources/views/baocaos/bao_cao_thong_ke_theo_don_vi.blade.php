...
<div class="container container-list report-statistic">
    ...
    <div class="row">
        ...
        <form class="form-filter-bao-cao justify-content-end" action="" method="GET" style="{{ ($start_time || $end_time) ? '' : 'display: none' }}">
        ...
        <ul class="nav nav-tabs">
            <li><a href="{{ route('baocaothongke.canhan') }}">Theo cá nhân</a></li>
            <li class="active"><a href="{{ route('baocaothongke.donvi') }}">Theo đơn vị</a></li>
        </ul>
        <br>
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr data-donvi-id="{{ $val->donvi_id }}">
                    ...
                    <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer ds-bao-cao bg-success" data-type="3">{{ $val->tongVBHoanThanhDungHan }}</td>
                    <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer ds-bao-cao bg-warning" data-type="4">{{ $val->tongVBHoanThanhQuaHan }}</td>
                    <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer ds-bao-cao bg-danger" data-type="5">{{ $val->tongVBChuaHoanThanh }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
<script type="text/javascript">
    ...
    $('.ds-bao-cao').click(function(event) {
        ...
        if(type == 1) $('#list-docs .modal-title').text('Danh sách văn bản chủ trì - đơn vị');
        else if(type == 2) $('#list-docs .modal-title').text('Danh sách văn bản phối hợp - đơn vị');
        else if(type == 3) $('#list-docs .modal-title').text('Danh sách văn bản hoàn thành đúng hạn - đơn vị');
        else if(type == 4) $('#list-docs .modal-title').text('Danh sách văn bản hàn thành quá hạn - đơn vị');
        else if(type == 5) $('#list-docs .modal-title').text('Danh sách văn bản chưa hoàn thành - đơn vị');
        loading_show();

        $.getJSON('{{ route("baocaothongke.danhsach") }}', {donvi_id: donvi_id, type: type, start_time: start_time, end_time: end_time}, function(json, textStatus) {
            loading_hide();
