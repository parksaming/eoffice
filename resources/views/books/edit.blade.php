                            ...
                            <div class="col-md-10" style="display: flex; flex-direction: column-reverse;">
                                <select class="form-control" name="donvi_id">
                                    ...
                                    @foreach ($donvis as $donvi)
                                        <option value="{{$donvi->id}}" {{ ($bookDetail->donvi_id == $donvi->id)? 'selected' : '' }}>{{$donvi->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="col-md-12 vanban">
                        <div class="row" style="display: flex">
                            ...
                            <div class="col-md-10" style="display: flex; height: 120px; flex-direction: column-reverse;">
                                <input type="text" class="form-control" name="donvi_email" data-role="tagsinput" value="{{ implode(',', $bookDetail->emails) }}">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row" style="display: flex">
                            ...
                            <div class="col-md-10" style="display: flex; flex-direction: column-reverse;">
                                <select name="user_id[]" class="form-control" multiple data-placeholder="Chọn cán bộ">
                                    @php $userIds = $bookDetail->userIds @endphp

                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" {{ in_array($user->id, $userIds)? 'selected' : '' }}>{{ sprintf('%s - %s - %s', $user->fullname, $user->email, $user->donvi_name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
