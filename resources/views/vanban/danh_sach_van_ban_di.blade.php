...
<div class="container container-list">
    <div class="row">
        ...
        @php
            ...
            $donviSoan = isset($_GET['donviSoan'])? $_GET['donviSoan'] : '';
            $donvi_nhan_vbdi  = isset($_GET['donvi_nhan_vbdi '])? $_GET['donvi_nhan_vbdi '] : '';
        ...
        @endphp
        <div class="container container-search {{ ($tukhoa === '' && $donviSoan === ''  && $donvi_nhan_vbdi === '' && $linhvucId === '' && $loaivanbanId === '' && $ngaybanhanhtu === '' && $ngaybanhanhden === '' && $ngayguitu === '' && $ngayguiden === '')? 'hidden' : '' }}">
            <div class="row">
                <div class="col-md-12">
                    <form method="GET" action="{{ route('danhsach.vanbandi') }}">
                               ...
                               <div class="row">
                                   ...
                                   <div class="col-md-6">
                                       <div class="row">
                                           ...
                                           <div class="col-md-9">
                                               @php $donviSoan = isset($_GET['donviSoan'])? $_GET['donviSoan'] : '' @endphp
                                               <input type="text" class="form-control" name="donviSoan" id ="donviSoan" value="{{$donviSoan}}">
                                           </div>
                                       </div>
                                   </div>
                               </div>
                               ...
                               <div class="row">
                                   ...
                                   <div class="col-md-6">
                                       <div class="row">
                                           ...
                                           <div class="col-md-9">
                                                @php $donvi_nhan_vbdi = isset($_GET['donvi_nhan_vbdi'])? $_GET['donvi_nhan_vbdi'] : '' @endphp
                                               <input type="text" class="form-control" name="donvi_nhan_vbdi" id ="donvi_nhan_vbdi" value="{{$donvi_nhan_vbdi}}">
                                           </div>
                                       </div>
                                   </div>
                               </div>
            ...
            <tbody>
                @if (sizeof($vanbans))
                    ...
                    <tr>
                        ...
                        <td>{{$vanban->donvi_nhan_vbdi? $vanban->donvi_nhan_vbdi : ''}}</td>
                        <td>
                            <a href="{{ route('chi_tiet_van_ban_di', $vanban->id) }}">{{$vanban->sovb}}</a>
                        </td>
                        <td>
                            <a href="{{ route('chi_tiet_van_ban_di', $vanban->id) }}">{{$vanban->kyhieu}}</a>
                        </td>
                        <td>{{$vanban->ngayky}}</td>
                        <td>{{$vanban->title}}</td>
                        <td>
                        ...
                        <td class="col-stt">
                            @if ($vanban->user_id == session('user')['id'])
                                <span style="padding:5px 10px"><a href="{{ route('edit_vanban_di', $vanban->id) }}" title="Sửa văn bản"><i class="fa fa-edit"></i></a></span>
                            @endif
                        </td>
                        <td class="col-stt">
                            @if ($vanban->user_id == session('user')['id'])
...
<script type="text/javascript">
    function deleteVanBan(id) {
        jConfirm('Bạn có muốn xóa văn bản?', 'Thông báo', function (r) {
            if (r) {
                ...
                let CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                $.post("{{ route('vanban.delete') }}", {ids:[id], _token:CSRF_TOKEN}, () => {
                    location.reload();
                }, 'json');
            }
        });
    ...
    $('#search').click(function(){
        ...
        var donviSoan = $('#donviSoan').val();
        var donvi_nhan_vbdi = $('#donvi_nhan_vbdi').val();
